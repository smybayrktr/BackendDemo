...
namespace Business
{
    public class UserManager : IUserService
    {
        ...
        public async Task<IResult> CreateANewPassword(CreateANewPasswordDto createANewPasswordDto)
        {
            ...
            if (user == null)
                return new ErrorResult(Messages.ForgotPasswordValueIsNotValid);
        ...
        public static IResult IsForgotPasswordValueUsed(User user)
        {
            if (user.IsForgotPasswordComplete)
                return new ErrorResult(Messages.ForgotPasswordValueIsUsed);
            else
        ...
        public static IResult IsForgotPasswordDateEnded(User user)
        {
            ...
            if (remainMin < -5)
            {
                return new ErrorResult(Messages.ForgotPasswordValueTimeIsEnded);
            }
        ...
        public async Task<IResult> ConfirmUser(string confirmValue)
        {
            ...
            if (user.IsConfirm)
            {
                return new ErrorResult(Messages.UserAlreadyConfirm);
            }

            user.IsConfirm = true;
            await _userDal.Update(user);
            return new SuccessResult(Messages.UserConfirmIsSuccesiful);
        }
        ...
        public async Task<IResult> SendForgotPasswordMail(string email)
        {
            ...
            return new SuccessResult(Messages.ForgotPasswordMailSendSuccessiful);
        }
        ...
        public IResult CheckForgotPasswordIsRequestActive(User user)
        {
            if (!user.IsForgotPasswordComplete)
            {
                if (user.ForgotPasswordRequestDate != null)
                {
                    ...
                    if (remainMin >= -5)
                    {
                        return new ErrorResult(Messages.AlreadySendForgotPasswordMail);
                    }
                }
            }
        ...
        public async Task<IResult> SendConfirmUserMail(string email)
        {
            ...
            return new SuccessResult(Messages.ConfirmUserMailSendSuccessiful);
        }
    }
}
